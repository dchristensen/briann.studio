...
Tuition is $<span children={data.price30min * 12} />/year which breaks down to $<span children={data.price30min} />/month for a 30 minute lesson or $<span children={data.price45min * 12} />/year and $<span children={data.price45min} />/month for a 45 minute lesson. Additional siblings receive a $<span children={data.siblingDiscount} /> discount per month per child.
